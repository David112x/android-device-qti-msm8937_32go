...
ifneq ($(strip $(BOARD_DYNAMIC_PARTITION_ENABLE)),true)
  ifeq ($(ENABLE_VENDOR_IMAGE), true)
      BOARD_VENDORIMAGE_PARTITION_SIZE := 398458880
  endif
  ...
  ifeq ($(ENABLE_AB), true)
    BOARD_SUPER_PARTITION_SIZE := 3003121664
  else
    BOARD_SUPER_PARTITION_SIZE := 1503657984
  endif
  BOARD_SUPER_PARTITION_GROUPS := qti_dynamic_partitions
  BOARD_QTI_DYNAMIC_PARTITIONS_SIZE := 1499463680
...
ifneq ($(wildcard kernel/msm-3.18),)
    ifeq ($(ENABLE_AB),true)
      ifeq ($(ENABLE_VENDOR_IMAGE), true)
        TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-3.18/recovery_AB_split_variant.fstab
      else
        TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-3.18/recovery_AB_non-split_variant.fstab
      endif
    else
      ifeq ($(ENABLE_VENDOR_IMAGE), true)
        TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-3.18/recovery_non-AB_split_variant.fstab
      else
        TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-3.18/recovery_non-AB_non-split_variant.fstab
      endif
    endif
    ...
    ifeq ($(ENABLE_AB),true)
      ifeq ($(ENABLE_VENDOR_IMAGE), true)
        ifeq ($(strip $(BOARD_DYNAMIC_PARTITION_ENABLE)),true)
          TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-4.9/recovery_AB_dynamic_variant.fstab
        else
          TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-4.9/recovery_AB_split_variant.fstab
        endif
      else
        TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-4.9/recovery_AB_non-split_variant.fstab
      endif
    else
      ifeq ($(ENABLE_VENDOR_IMAGE), true)
        ifeq ($(strip $(BOARD_DYNAMIC_PARTITION_ENABLE)),true)
          TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-4.9/recovery_non-AB_dynamic_variant.fstab
        else
          TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-4.9/recovery_non-AB_split_variant.fstab
        endif
      else
        TARGET_RECOVERY_FSTAB := device/qcom/msm8937_32go/fstabs-4.9/recovery_non-AB_non-split_variant.fstab
      endif
    endif
else
...
ifeq ($(strip $(TARGET_KERNEL_VERSION)), 4.9)
     BOARD_KERNEL_CMDLINE := mem=1G console=ttyMSM0,115200,n8 androidboot.console=ttyMSM0 androidboot.hardware=qcom androidboot.memcg=true user_debug=30 msm_rtb.filter=0x237 ehci-hcd.park=3 androidboot.bootdevice=7824900.sdhci lpm_levels.sleep_disabled=1 earlycon=msm_hsl_uart,0x78B0000 vmalloc=300M androidboot.usbconfigfs=true cgroup.memory=nokmem,nosocket loop.max_part=7
else ifeq ($(strip $(TARGET_KERNEL_VERSION)), 3.18)
     BOARD_KERNEL_CMDLINE := console=ttyHSL0,115200,n8 androidboot.console=ttyHSL0 androidboot.hardware=qcom androidboot.memcg=true user_debug=30 msm_rtb.filter=0x237 ehci-hcd.park=3 androidboot.bootdevice=7824900.sdhci lpm_levels.sleep_disabled=1 earlycon=msm_hsl_uart,0x78B0000 loop.max_part=7
endif

#BOARD_KERNEL_CMDLINE += androidboot.selinux=permissive

BOARD_SECCOMP_POLICY := device/qcom/msm8937_32go/seccomp

BOARD_EGL_CFG := device/qcom/msm8937_32go/egl.cfg
